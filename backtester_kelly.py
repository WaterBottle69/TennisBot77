...
try:
    ...
    _HAS_XGB = False
try:
    from shap_analyzer import walk_forward_ablation, analyze_xgb
    _HAS_SHAP = True
except ImportError:
    _HAS_SHAP = False
...
PDF_OUT        = os.path.join(os.path.dirname(__file__), "backtest_kelly_analysis.pdf")
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        # ── Differential features (no post-match info) ────────────────────
...
def simulate_betting(model_prob: np.ndarray,
    ...
    for mp, mkp_raw, outcome in zip(model_prob, market_prob, outcomes):
        ...
        # Confidence-Tiered scaling based on model's winning probability
        if mp >= 0.70:
            tier_fraction = 0.40
        elif mp >= 0.60:
            tier_fraction = 0.25
        ...
        fraction_to_wager = f_star * tier_fraction * (kelly_frac / 0.25) # Scale relative to user's config
        fraction_to_wager = min(fraction_to_wager, 0.20) # Increased to 20% for smarter aggressive plays
        
        bet_cost = bankroll * fraction_to_wager
        net_profit = (bet_cost / mkp_adj) * (1.0 - mkp_adj)
...
def main():
    ...
    # ── BASELINE: existing pre-trained model (14 features) ───────────────
    ...
    log.info(f"[BASELINE] Accuracy : {accuracy_score(y_true, y_pred)*100:.2f}%")
    log.info(f"[BASELINE] ROC-AUC  : {roc_auc_score(y_true, y_prob):.4f}")
    log.info(f"[BASELINE] Brier    : {brier_score_loss(y_true, y_prob):.4f}")
    ...
    log.info(f"[BASELINE] Betting ROI: {betting['roi']:+.1f}%  |  {betting['n_bets']} bets  |  "
             f"win rate {betting['win_rate']:.1f}%  |  final ${betting['bankroll_final']:,.0f}")

    # ── EXPANDED: inline retrain with 21 features ─────────────────────────
    ...
    EXP_FEAT_COLS = [
        ...
        "Height_Diff", "Age_Diff", "Rank_Diff", "Rank_Points_Diff", "Elo_Diff",
        "Surface_Height_Grass", "Age_Diff_Sq",
    ]

    train_df_exp = feat_df[train_mask].reset_index(drop=True)
    test_df_exp  = feat_df[test_mask].reset_index(drop=True)
    X_train_exp  = train_df_exp[EXP_FEAT_COLS]
    y_train_exp  = train_df_exp["target"].values
    X_test_exp   = test_df_exp[EXP_FEAT_COLS]
    y_true_exp   = test_df_exp["target"].values

    # Impute NaN with training-set median (height/age missing for some rows)
    train_medians = X_train_exp.median()
    X_train_exp   = X_train_exp.fillna(train_medians)
    X_test_exp    = X_test_exp.fillna(train_medians)

    log.info(f"Training expanded model on {len(X_train_exp):,} rows, {len(EXP_FEAT_COLS)} features…")
    base_clf = GradientBoostingClassifier(
        n_estimators=200, max_depth=4, learning_rate=0.05,
        min_samples_leaf=30, subsample=0.8, max_features=0.8,
        random_state=42
    )
    # Isotonic calibration improves Brier score by aligning predicted probs to observed rates
    exp_model = CalibratedClassifierCV(base_clf, method="isotonic", cv=3)
    exp_model.fit(X_train_exp, y_train_exp)

    y_prob_exp = exp_model.predict_proba(X_test_exp)[:, 1]
    y_pred_exp = (y_prob_exp >= 0.5).astype(int)
    acc_exp    = accuracy_score(y_true_exp, y_pred_exp)
    auc_exp    = roc_auc_score(y_true_exp, y_prob_exp)
    brier_exp  = brier_score_loss(y_true_exp, y_prob_exp)

    log.info(f"[EXPANDED] Accuracy : {acc_exp*100:.2f}%")
    log.info(f"[EXPANDED] ROC-AUC  : {auc_exp:.4f}")
    log.info(f"[EXPANDED] Brier    : {brier_exp:.4f}")

    betting_exp = simulate_betting(
        model_prob  = y_prob_exp,
        market_prob = test_df_exp["elo_prob_p1"].values,
        outcomes    = y_true_exp,
    )
    log.info(f"[EXPANDED] Betting ROI: {betting_exp['roi']:+.1f}%  |  {betting_exp['n_bets']} bets  |  "
             f"win rate {betting_exp['win_rate']:.1f}%  |  final ${betting_exp['bankroll_final']:,.0f}")

    # ── generate PDF ──────────────────────────────────────────────────────
    # generate_pdf(test_df, y_true, y_prob, y_pred, betting, feature_names, model, PDF_OUT)
    log.info("PDF generation skipped as requested.")

    print("\n" + "="*60)
    print("BASELINE RESULTS  (14 features, pre-trained model)")
    print("="*60)
    print(f"Accuracy :  {accuracy_score(y_true, y_pred)*100:.2f}%")
    print(f"ROC-AUC  :  {roc_auc_score(y_true, y_prob):.4f}")
    print(f"Brier    :  {brier_score_loss(y_true, y_prob):.4f}")
    print(f"Betting ROI: {betting['roi']:+.1f}%  |  {betting['n_bets']} bets  |  win rate {betting['win_rate']:.1f}%")
    print(f"Ending Bankroll: ${betting['bankroll_final']:,.2f}")

    print("\n" + "="*60)
    print("EXPANDED RESULTS  (21 features, retrained + calibrated)")
    print("="*60)
    print(f"Accuracy :  {acc_exp*100:.2f}%  (Δ {(acc_exp - accuracy_score(y_true,y_pred))*100:+.2f}pp)")
    print(f"ROC-AUC  :  {auc_exp:.4f}  (Δ {auc_exp - roc_auc_score(y_true,y_prob):+.4f})")
    print(f"Brier    :  {brier_exp:.4f}  (Δ {brier_exp - brier_score_loss(y_true,y_prob):+.4f})")
    print(f"Betting ROI: {betting_exp['roi']:+.1f}%  |  {betting_exp['n_bets']} bets  |  win rate {betting_exp['win_rate']:.1f}%")
    print(f"Ending Bankroll: ${betting_exp['bankroll_final']:,.2f}")
    print("="*60 + "\n")

# ─────────────────────────────────────────────────────────────────────────────
# Walk-Forward SHAP Ablation (Phase 2 — information.md)
# ─────────────────────────────────────────────────────────────────────────────

def run_shap_ablation(df: pd.DataFrame, feature_cols: list, label_col: str = "winner_id"):
    """
    Run Walk-Forward SHAP ablation on live ATP features.

    Prevents look-ahead bias by strictly splitting: train on data up to year T,
    test on year T+1. Never uses K-fold (which leaks future match outcomes).

    Feature groups to ablate (remove one group at a time):
      - serve_pct:    first serve % and points won on serve
      - break_pts:    break point conversion and save rates
      - aces_df:      aces and double faults
      - serve_speed:  average and max serve speed
    """
    if not _HAS_SHAP:
        log.warning("[SHAP] shap_analyzer not available — skipping ablation.")
        return {}

    try:
        X = df[feature_cols].fillna(0).values
        y = (df[label_col] == df[label_col].iloc[0]).astype(int).values
    except KeyError as e:
        log.error(f"[SHAP] Missing column: {e}")
        return {}

    if len(X) < 50:
        log.warning(f"[SHAP] Too few samples ({len(X)}) for ablation — need ≥ 50.")
        return {}

    # Walk-forward split: 80% train, 20% test
    split = int(len(X) * 0.80)

    # Map feature group names → column index ranges
    n = len(feature_cols)
    serve_idx  = [i for i, f in enumerate(feature_cols) if "serve_pct"   in f or "1st_serve" in f]
    break_idx  = [i for i, f in enumerate(feature_cols) if "break"       in f]
    ace_idx    = [i for i, f in enumerate(feature_cols) if "ace"         in f or "double_fault" in f]
    speed_idx  = [i for i, f in enumerate(feature_cols) if "speed"       in f]
    # Fallback: distribute evenly if no naming conventions match
    if not serve_idx:  serve_idx  = list(range(0, max(1, n // 4)))
    if not break_idx:  break_idx  = list(range(max(1, n // 4), max(2, n // 2)))
    if not ace_idx:    ace_idx    = list(range(max(2, n // 2), max(3, 3 * n // 4)))
    if not speed_idx:  speed_idx  = list(range(max(3, 3 * n // 4), n))

    feature_groups = {
        "serve_pct":   serve_idx,
        "break_pts":   break_idx,
        "aces_df":     ace_idx,
        "serve_speed": speed_idx,
    }

    log.info(f"[SHAP] Running walk-forward ablation: {split} train / {len(X)-split} test samples")
    results = walk_forward_ablation(
    ...
    # Also run global SHAP plots on test set
    try:
        import xgboost as xgb
        model = xgb.XGBClassifier(n_estimators=300, max_depth=4, learning_rate=0.05,
                                   subsample=0.8, colsample_bytree=0.8, verbosity=0)
        model.fit(X[:split], y[:split])
        analyze_xgb(model=model, X_test=X[split:], feature_names=feature_cols)
    except Exception as e:
        log.warning(f"[SHAP] Global plot generation failed: {e}")

    return results


if __name__ == "__main__":
    log.info("Walk-Forward Tennis Backtest starting...")
    try:
        df = load_atp_matches(ATP_DIR)
        log.info(f"Loaded {len(df):,} matches from {ATP_DIR}")
    except FileNotFoundError as e:
        log.warning(f"ATP data not found at {ATP_DIR}: {e}")
        log.info("Run with --demo to use synthetic data, or place atp_matches_*.csv in ~/Downloads/tennis_atp-master/")

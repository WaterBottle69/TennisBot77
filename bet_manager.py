...
class BetManager:
    ...
    async def evaluate_and_act(
        ...
        # --- Entry logic ---
        # Adaptive MIN_EDGE: use controller's live value if available, else config
        effective_min_edge = adaptive.current_min_edge if adaptive else max(self.cfg.MIN_EDGE, 0.02)
        ...
        def should_bet_meta(model_p, market_price, edge, flow: MarketSignal):
            if flow.direction == FlowDirection.FADE:
                return False, f"[FLOW] FADE signal — market moving against our edge"
            if edge < effective_min_edge:
                return False, f"Edge {edge:+.3f} below adaptive threshold {effective_min_edge:.3f}"
        ...
        should_yes, reason_yes = should_bet_meta(p_a, yes_price, edge_yes, flow_yes)
        should_no, reason_no   = should_bet_meta(p_b, no_price,  edge_no,  flow_no)
    ...
    async def place_predictive_limit_order(
        ...
        # We place a limit at the CURRENT price (before the point) targeting the
        # branch where our model probability jumps above the existing ask.
        # Specifically: if A winning the point takes our model prob to prob_if_win
        # and current yes_price < prob_if_win - fee, the limit at yes_price gives
        # us the edge that a market order would only get AFTER the move.
        candidates = []
        ...
        if edge_yes_limit >= effective_min_edge:
        ...
        if edge_no_limit >= effective_min_edge:

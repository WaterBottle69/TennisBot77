...
class MarketMonitor:
    ...
    async def _poll_and_update(self):
        ...
        # Extract prices — Kalshi returns cents (0-99)
        yes_bid = data.get("yes_bid", 50) / 100
        yes_ask = data.get("yes_ask", 50) / 100
        yes_price = (yes_bid + yes_ask) / 2

        # Volume imbalance (if available)
        vol_yes = data.get("yes_volume",  0) or 0
        vol_no  = data.get("no_volume",   0) or 0
